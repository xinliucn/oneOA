...
export interface TodoItem extends Record<string, any> {
  id?: string | number
  code?: string
  title?: string
  referenceNo?: string
...
const getStringValue = (record: Record<string, any>, key: string) => {
...
const unwrapWorkflowAttachmentPayload = (payload: any): any => {
  if (!payload) {
    return []
  }
...
const normalizeWorkflowFormAttachments = (payload: any): WorkflowFormAttachment[] => {
...
export const useToDoData = () => {
  ...
  const getApprovals = async (query: TodoQuery = {}) => {
    const data = await fetchTodoList<TodoItem>('approvals', query)
    approvals.value = data
    return data
  }

  const getRequests = async (query: TodoQuery = {}) => {
    const data = await fetchTodoList<TodoItem>('requests', query)
    requests.value = data
    return data
  }
  ...
  const getFormAttachments = async (requestid: string) => {
    try {
      const response = await $fetch<any>('/api/todo/formAttachments', {
        method: 'POST',
        body: {
          requestid,
        },
